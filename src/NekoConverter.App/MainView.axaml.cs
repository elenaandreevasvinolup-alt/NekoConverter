...
public partial class MainView : UserControl
{
    ...
    public MainView()
    {
        ...
        // Меню собирается после подключения к окну: до этого TopLevel ещё нет,
        // и вешать системную строку не на что.
        AttachedToVisualTree += (_, _) => BuildNativeMenu();
    ...
    private void BuildNativeMenu()
    {
        ...
        // Меню вешается на окно, а не на сам UserControl: системная строка macOS
        // принадлежит окну. На телефоне окна нет — там меню и не нужно.
    ...
    /// <summary>
    /// Окно сравнения — отдельное окно. На телефоне открыть второе окно нельзя,
    /// поэтому кнопка «Сравнить» там не показывается вовсе: кнопка, которая
    /// ничего не делает, хуже её отсутствия.
    /// </summary>
    private static bool CanOpenCompareWindow => !IsMobilePlatform;

    /// <summary>
    /// Буфер обмена принадлежит окну, а не содержимому: у UserControl такого
    /// свойства нет. Берём его у ближайшего TopLevel — на телефоне это экран.
    /// </summary>
    private IClipboard? Clipboard => TopLevel.GetTopLevel(this)?.Clipboard;

    /// <summary>Выбор файлов тоже живёт на уровне окна, а не содержимого.</summary>
    private IStorageProvider StorageProvider =>
        TopLevel.GetTopLevel(this)?.StorageProvider
        ?? throw new InvalidOperationException(
            "Окно ещё не подключено: выбрать файл можно только после показа.");

    /// <summary>Закрывает окно. На телефоне окна нет, и закрывать нечего.</summary>
        ...
        foreach (var group in _groups)
        {
            foreach (var (file, status, details) in group.Rows)
            {
                ...
                if (result.Success && result.OutputPath is { Length: > 0 } output)
                {
                    ...
                    // Кнопка «Сравнить» открывает отдельное окно, а на телефоне
                    // второго окна нет — там её показывать нечем.
                    if (file.CompareButton is { } button && CanOpenCompareWindow)
                    {
